...
def build_thoth_graph():
    ...
    workflow.add_node("infrastructure", tool_node)  # Prefect/Postgres Bridge
    workflow.add_node("assessment", analysis_node)  # Applying Doctrine (Policy)
    workflow.add_node("finalize", finalization_node)  # Cleanup & Success report
